 ...
 *
 * JSON Format Usage:
 * To control the relay, send JSON messages to the MQTT control topic:
 *
 * Relay Control:
 * {"relay": 1}     - Turn relay ON (LED ON)
 * {"relay": 0}     - Turn relay OFF (LED OFF)
 *
 * Response Format:
 * The device will publish its current state to the telemetry topic:
 * {"relay": 1, "led_status": "ON"}
 * {"relay": 0, "led_status": "OFF"}
 *
 * Note: The LED connected to relay NC pin will show the relay status
 *
